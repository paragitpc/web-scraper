...
SOURCE = "dgi_consultas_no_vinculantes"
LISTING_URL = "https://www.impo.com.uy/bases/consultas-dgi-no-vinculantes"
DETAIL_PREFIX = "/bases/consultas-dgi-no-vinculantes/"
MIN_BODY_TEXT = 200
...
async def process_url(client, url, storage: LocalStorage, already_done: set[str]) -> str:
    ...
    storage.append_index_record(
        source=SOURCE, key=key, url=url, relative_path=rel_html,
...
def main() -> None:
    parser = base_parser("Scraper DGI Consultas No Vinculantes (via IMPO)")
