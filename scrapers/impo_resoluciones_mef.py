...
SOURCE = "impo_resoluciones_mef"
URL_TEMPLATE = "https://www.impo.com.uy/bases/resoluciones-mef/{n}-{y}/1"
...
async def process_resolution(
    ...
    try:
        ...
        print(f"  [error] res mef {key}: {type(exc).__name__}: {exc}")
        return "error"
    ...
    storage.append_index_record(
        ...
        extra={"title": title, "year": y, "number": n},
    )
    print(f"  [ok] res mef {key}  ({len(text):,} chars)")
    return "ok"
    ...
    async with make_async_client(headers={"Accept": "text/html,*/*"}) as client:
        for y in range(from_year, to_year + 1):
            for n in range(from_n, to_n + 1):
                result = await process_resolution(client, n, y, storage, already_done)
...
def main() -> None:
    parser = base_parser("Scraper IMPO Resoluciones MEF")
    parser.add_argument("--from-year", type=int, required=True)
    parser.add_argument("--to-year", type=int, required=True)
    parser.add_argument("--from-n", type=int, default=1)
    parser.add_argument("--to-n", type=int, default=800)
